...
- (void)setup {
    if (!self.indicatorView) {
        XHRadarIndicatorView *indicatorView = [[XHRadarIndicatorView alloc] initWithFrame:(CGRect){0,0,self.bounds.size.width,self.bounds.size.height}];
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i=0; i<sectionsNum ; i++) {
        /*画圆*/
        //边框圆
        CGContextSetRGBStrokeColor(context, 170/255, 170/255, 170/255, (1-(float)i/(sectionsNum + 1))*0.2);//画笔线的颜色(透明度渐变)
        CGContextSetLineWidth(context, 1);//线的宽度
        //void CGContextAddArc(CGContextRef c,CGFloat x, CGFloat y,CGFloat radius,CGFloat startAngle,CGFloat endAngle, int clockwise)1弧度＝180°/π （≈57.3°） 度＝弧度×180°/π 360°＝360×π/180 ＝2π 弧度
        // x,y为圆点坐标，radius半径，startAngle为开始的弧度，endAngle为 结束的弧度，clockwise 0为顺时针，1为逆时针。
        CGContextAddArc(context, self.center.x, self.center.y+64, sectionRadius - 5*(sectionsNum - i - 1), 0, 2*M_PI, 0); //添加一个圆
    ...
    if (self.textLabel) {
        self.textLabel.frame = CGRectMake(0, self.center.y +128+ ([UIScreen mainScreen].bounds.size.height)/3.3, rect.size.width, 60);
        self.textLabel.textColor = kBarNormalColor;
